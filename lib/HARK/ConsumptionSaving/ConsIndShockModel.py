...
#        PermShkNow[newborn] = 1.0
    ...
    def checkConditions(self,verbose=False):
        ...
        for i in range(len(self.PermShkDstn[0][1])):
            exp_psi_inv=exp_psi_inv+(1.0/self.PermShkCount)*(self.PermShkDstn[0][1][i])**(-1)

        #Get expected psi to the power one minus CRRA
        for i in range(len(self.PermShkDstn[0][1])):
            exp_psi_to_one_minus_rho=exp_psi_to_one_minus_rho+(1.0/self.PermShkCount)*(self.PermShkDstn[0][1][i])**(1-self.CRRA)
        ...
        if GIC<1:
            ...
            print('The given type violates the growth impatience condition with the supplied parameter values; the GIC is: %2.4f' % (GIC))
            print('Therefore, a nondegenerate solution may not be available. See Table 3 in "Theoretical Foundations of Buffer Stock Saving" (Carroll, 2011) to check which conditions are sufficient for a nondegenerate solution.')
        if verbose:
